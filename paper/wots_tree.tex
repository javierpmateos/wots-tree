...
\paragraph{Hash-based signatures.} The concept of hash-based digital signatures originates with Lamport~\cite{lamport1979constructing}, who showed that any one-way function suffices for one-time signatures. Merkle~\cite{merkle1989certified} extended this to many-time signatures via authenticated binary trees. Subsequent work introduced Winternitz chains to compress Lamport signatures, culminating in WOTS+~\cite{hulsing2013wots} with its tight security reduction under second-preimage resistance. XMSS~\cite{buchmann2011xmss} and its multi-tree variant XMSS$^{MT}$~\cite{rfc8391} combine WOTS+ with Merkle trees to obtain stateful many-time signatures, standardized in RFC~8391~\cite{rfc8391} and NIST SP~800-208~\cite{nist2020sp800208}. Drake et al.~\cite{drake2025multisig} generalize the XMSS framework for multi-signature applications in Ethereum's consensus layer, providing tighter security bounds that bypass random oracle dependence. Khovratovich, Kudinov, and Wagner~\cite{khovratovich2025hypercube} revisit the Winternitz encoding process and introduce non-injective incomparable encodings that achieve 20--40\% improvement in verification cost at the same signature size and security level; these encodings can be plugged directly into XMSS and SPHINCS+. Drake et al.~\cite{drake2025leansig} instantiate this encoding within the multi-signature framework for Ethereum. These works address the fundamental size-time tradeoff in Winternitz chains and are complementary to our contribution, which focuses on parameter selection and deployment within Bitcoin's UTXO model. In particular, the encoding optimizations of~\cite{khovratovich2025hypercube} could be applied to WOTS-Tree's chain evaluations to further reduce verification cost.
...
\paragraph{Bitcoin-specific optimizations.} Kudinov and Nick~\cite{kudinov2025} provide a comprehensive analysis of hash-based signatures for Bitcoin, introducing SPHINCS+C (using non-injective compressed encodings~\cite{kudinov2022sphincsc}) and PORS+FP (an optimized few-time signature replacement for FORS). Their parameter search yields witnesses of 3,128--4,704~bytes with signing-time grinding of $\approx 2^{16}$ hash evaluations. The SHRINCS proposal~\cite{nick2025shrincs} combines a small stateful scheme with a stateless ``parachute'' fallback, achieving 324~bytes optimally but expanding to $\sim$3.4~KB upon state loss.
...
\begin{enumerate}[nosep]
    \item \textbf{Preimage resistance}: Finding $x$ such that $H(x) = y$ requires $\Omega(2^\lambda)$ evaluations by a generic attack.
